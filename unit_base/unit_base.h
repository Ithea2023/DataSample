...
public:
  ...
  constexpr bool
  operator==(const Unit_T& other) const {
  ...
  constexpr Unit_T
  RoundTo(const Unit_T& resolution) const {
    RTC_DCHECK(IsFinite());
  ...
  constexpr Unit_T
  RoundUpTo(const Unit_T& resolution) const {
    RTC_DCHECK(IsFinite());
    RTC_DCHECK(resolution.IsFinite());
    RTC_DCHECK_GT(resolution.value_, 0);
  ...
  constexpr Unit_T
  RoundDownTo(const Unit_T& resolution) const {
    RTC_DCHECK(IsFinite());
    RTC_DCHECK(resolution.IsFinite());
    RTC_DCHECK_GT(resolution.value_, 0);
...
protected:
  ...
  template<typename T, typename std::enable_if<std::is_integral<T>::value>::type* = nullptr>
  static constexpr Unit_T
  FromFraction(int64_t denominator, T value) {
  ...
  ToValue() const {
    return IsPlusInfinity() ? std::numeric_limits<T>::infinity() : IsMinusInfinity() ? -std::numeric_limits<T>::infinity() : value_;
  }

  //ToValueOr
  template<typename T>
  constexpr T
  ToValueOr(T fallback_value) const {
  ...
  constexpr int64_t
  ToFractionOr(int64_t fallback_value) const {
    return IsFinite() ? Unit_T::one_sided ? DivRoundPositionToNearest(value_, Denominator) : DivRoundToNearset(value_, Denominator) : fallback_value;
  }
...
private:
  ...
  constexpr const Unit_T&
  AsSubClassRef() const {
    return static_cast<const Unit_T&>(*this);
  }

  static constexpr int64_t
  DivRoundPositionToNearest(int64_t n, int64_t d) {
    return (n + d / 2) / d;
  }

  static constexpr int64_t
  DivRoundToNearset(int64_t n, int64_t d) {
...
public:
  constexpr Unit_T
  Clamped(Unit_T min_value, Unit_T max_value) const {
    return std::max(min_value,
                    std::min(UnitBase<Unit_T>::AsSubClassRef(), max_value));
  }

  constexpr void
  Clamp(Unit_T min_value, Unit_T max_value) {
  ...
  constexpr Unit_T
  operator*(int64_t scalar) const {
    return UnitBase<Unit_T>::FromValue(this->ToValue() * scalar);
  }

  constexpr Unit_T
  operator*(int32_t scalar) const {
    return UnitBase<Unit_T>::FromValue(this->ToValue() * scalar);
  }
